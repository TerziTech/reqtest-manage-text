...
import rehierCommand from './rehier.js';

const program = new Command();
program.name('rmt').description('CLI for managing requirements and tests');
...
function removeReqtest() {
  ...
  rl.question('Enter the name of the reqtest object to remove: ', name => {
    if (!reqtestNames.includes(name)) {
      ...
      const updatedContent = fs.readFileSync(filePath, 'utf-8').split("\n\n").filter(block => !block.includes(`.'. ${name}`)).join("\n\n");
...
program.command('list').description('List all reqtest objects in the project.rmt file').option('--verbose', 'Print the full details of each reqtest object').action(options => {
  const filePath = './project.rmt';
  if (!fs.existsSync(filePath)) {
    console.error('Error: project.rmt file not found. Please run "rmt init" to create one.');
    return;
...
const removeCommand = new Command('remove').description('Remove a reqtest object from the project.rmt file with tab completion').action(() => {
  removeReqtest();
});
program.addCommand(removeCommand);
const detailsCommand = new Command('details').description('View details of a specific reqtest object in the project.rmt file').action(() => {
  detailsReqtest();
});
program.addCommand(detailsCommand);
const editCommand = new Command('edit').description('Edit a specific reqtest object in the project.rmt file').action(() => {
  editReqtest();
});
program.addCommand(editCommand);
const treeCommand = new Command('tree').description('Display and manage the hierarchical structure of reqtest objects in the project.rmt file').action(() => {
  treeReqtest();
});
program.addCommand(treeCommand);
program.addCommand(new Command('rehier')
    .description('Recalculate the hierarchy of reqtest objects in the project.rmt file')
    .action(rehierCommand));
