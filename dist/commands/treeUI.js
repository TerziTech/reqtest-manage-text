...
const TreeUI = ({
  ...
  useInput((input, key) => {
    if (!isEditing) {
      if (key.upArrow) {
        ...
        const updatedTree = [...tree.slice(0, cursor + 1), newItem, ...tree.slice(cursor + 1)];
        setTree(updatedTree);
  ...
  const saveChanges = newItemName => {
    try {
      execSync(`npx rmt add --name "${newItemName}"`, {
        stdio: 'inherit'
      });
  ...
  }, /*#__PURE__*/React.createElement(Box, null, /*#__PURE__*/React.createElement(Text, null, "Use arrow keys to navigate. `ctrl + q' to quit. `ctrl + n' to insert a new item. Press Enter to edit. Press ctrl + s to save.")), message && /*#__PURE__*/React.createElement(Box, null, /*#__PURE__*/React.createElement(Text, {
    color: "yellow"
  }, message)), tree.map((reqtest, index) => /*#__PURE__*/React.createElement(Box, {
    key: index,
    flexDirection: "row"
  }, /*#__PURE__*/React.createElement(Text, {
    color: cursor === index ? 'green' : 'white'
  }, cursor === index ? '> ' : '  ', generateHierarchyNumber(index), " ", reqtest.name), cursor === index && isEditing && /*#__PURE__*/React.createElement(TextInput, {
    value: editValue,
